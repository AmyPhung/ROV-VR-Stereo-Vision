...
for pt in raw_points:
    ...
    #     break
    #
    # if i%12 != 0 and i%12 != 1 and i%12 != 2:
...
rvecsR = [np.zeros((1, 1, 3), dtype=np.float64) for i in range(len(cornersR))]
tvecsR = [np.zeros((1, 1, 3), dtype=np.float64) for i in range(len(cornersR))]

retR, mtxR, distR, rvecsR, tvecsR = \
    cv2.calibrateCamera(obj_pts, img_ptsR, imgR_gray.shape[::-1], K_R, D_R)#,

# # New 
# new_mtxL = cv2.getOptimalNewCameraMatrix(mtxL,
#                               distL,
#                               grayL.size(),
#                               1,
#                               grayL.size(),
